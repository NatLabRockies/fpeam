...
class Data(pd.DataFrame):
    ...
    def backfill(self, column, value=0):
        ...
        if self[column].isna().any():
            ...
            LOGGER.warning(
                "%s of %s data values in %s.%s were backfilled as %s"
                % (_count_missing, _count_total, _dataset, column, value)
            )
...
class Equipment(Data):
    COLUMNS = (
        {"name": "feedstock", "type": str, "index": True, "backfill": None},
...
class FeedstockLossFactors(Data):
    ...
    def __init__(self, df=None, fpath=None, columns=None, backfill=True):
        super(FeedstockLossFactors, self).__init__(
            df=df, fpath=fpath, columns=columns, backfill=backfill
        )
...
class ResourceDistribution(Data):
    ...
    def __init__(self, df=None, fpath=None, columns=None, backfill=True):
        super(ResourceDistribution, self).__init__(
            df=df, fpath=fpath, columns=columns, backfill=backfill
        )
        ...
        if fpath is not None:
            ...
            if "region" not in columns:
                ...
                if "region" in _header:
                    ...
                    columns["region"] = str
        super(EmissionFactor, self).__init__(df=df, fpath=fpath, columns=columns, backfill=backfill)
...
class FugitiveDustFactors(Data):
    ...
    def __init__(self, df=None, fpath=None, columns=None, backfill=True):
        super(FugitiveDustFactors, self).__init__(
            df=df, fpath=fpath, columns=columns, backfill=backfill
        )
...
class SiltContent(Data):
    COLUMNS = (
        {"name": "st_name", "type": str, "index": True, "backfill": None},
...
class FugitiveDustOnroadConstants(Data):
    COLUMNS = (
        {"name": "constant", "type": str, "index": True, "backfill": None},
    ...
    def __init__(self, df=None, fpath=None, columns=None, backfill=True):
        super(FugitiveDustOnroadConstants, self).__init__(
            df=df, fpath=fpath, columns=columns, backfill=backfill
        )
...
class NONROADEquipment(Data):
    ...
    def __init__(self, df=None, fpath=None, columns=None, backfill=True):
        super(NONROADEquipment, self).__init__(
            df=df, fpath=fpath, columns=columns, backfill=backfill
        )
...
class TransportationGraph(Data):
    COLUMNS = (
        {"name": "edge_id", "type": int, "index": True, "backfill": None},
    ...
    def __init__(self, df=None, fpath=None, columns=None, backfill=True):
        super(TransportationGraph, self).__init__(
            df=df, fpath=fpath, columns=columns, backfill=backfill
        )
...
class TransportationNodeLocations(Data):
    COLUMNS = (
        {"name": "node_id", "type": int, "index": True, "backfill": None},
        {"name": "x", "type": float, "index": False, "backfill": None},
        {"name": "y", "type": float, "index": False, "backfill": None},
    )

    def __init__(self, df=None, fpath=None, columns=None, backfill=True):
        super(TransportationNodeLocations, self).__init__(
            df=df, fpath=fpath, columns=columns, backfill=backfill
        )
...
class RegionFipsMap(Data):
    ...
    def validate(self):
        try:
            ...
            raise ValueError(
                "region_fips_map data must have only 1 FIPS per region and 1 region per FIPS"
            )
        else:
            return True
...
class StateFipsMap(Data):
    COLUMNS = (
        {"name": "state_abbreviation", "type": str, "index": True, "backfill": None},
...
class AVFT(Data):
    COLUMNS = (
        {"name": "sourceTypeID", "type": int, "index": True, "backfill": None},
        ...
        if fpath is not None:
            ...
            for col, dtype in self.OPTIONAL_COLUMNS.items():
                if col in df.columns:
                    columns[col] = dtype
        super(GeophysicalContext, self).__init__(
            df=df, fpath=fpath, columns=columns, backfill=backfill
        )
