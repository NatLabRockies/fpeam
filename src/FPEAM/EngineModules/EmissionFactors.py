...
from ..EmissionFactorProviders import TableProvider
...
class EmissionFactors(Module):
    ...
    def __init__(self, config, equipment, production, backfill=True, **kvals):
        ...
        self.emission_factors = EmissionFactor(
            fpath=self.config.get("emission_factors"), backfill=backfill
        )
        ...
        if not _unexpected_units.empty:
            LOGGER.warning(
                ...
                _unexpected_units[
                    ["resource", "resource_subtype", "unit_numerator", "unit_denominator"]
                ].to_dict("records"),
            )

        # Resource subtype distribution, Units: unit-less fraction
        self.resource_distribution = ResourceDistribution(
            fpath=self.config.get("resource_distribution"), backfill=backfill
        )
        ...
        _factors_merge = self.resource_distribution.merge(
            self.emission_factors, on=["resource", "resource_subtype"]
        )
        ...
        self.overall_factors = _factors_merge.groupby(
            _group_cols, as_index=False, dropna=False
        ).sum()

        if self._has_region_factors:
            LOGGER.info(
                "EmissionFactors loaded with region-keyed factors; "
        ...
        if _provider_name == "table":
            ...
            if _provider_cls is None:
                LOGGER.warning(
                    'Unknown provider "%s"; falling back to TableProvider.', _provider_name
                )
                self._provider = TableProvider(self.overall_factors)
            else:
    ...
    def get_emissions(self):
        ...
        _base = self.production[_prod_rows][_prod_columns].merge(
            self.equipment[_equip_columns], on=_idx, suffixes=["_prod", "_equip"]
        )
        ...
        if not isinstance(self._provider, TableProvider):
            ...
            _regional_factors = self.overall_factors[self.overall_factors["region"].notna()][
                _regional_cols
            ].copy()

            # rows without a region value (national defaults)
            _national_factors = self.overall_factors[self.overall_factors["region"].isna()][
                _factors_columns
            ]
            ...
            _covered_keys = _df_regional[
                ["feedstock", "resource", "region_production"]
            ].drop_duplicates()
            ...
            _base_national = _base_national[_base_national["_merge"] == "left_only"].drop(
                columns=["_merge"]
            )
        ...
        _df = _df.assign(
            pollutant_amount=_df["overall_rate"] * _df["feedstock_amount"] * _df["rate"]
        )
