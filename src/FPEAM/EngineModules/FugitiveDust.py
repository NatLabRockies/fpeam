...
from ..Data import FugitiveDustFactors, SiltContent, FugitiveDustOnroadConstants
...
class FugitiveDust(Module):
    ...
    def __init__(
        ...
        **kvals,
        ...
        self.fugitive_dust_factors = FugitiveDustFactors(
            fpath=self.config.get("fugitive_dust_factors"), backfill=backfill
        )
        self.silt_content = SiltContent(fpath=self.config.get("silt_content"), backfill=backfill)
        self.fugitive_dust_onroad_constants = FugitiveDustOnroadConstants(
            fpath=self.config.get("fugitive_dust_onroad_constants"), backfill=backfill
        )
    ...
    def get_onroad_fugitivedust(self):
        ...
        self.feedstock_loss_factors.eval("dry_matter_remaining = 1 - dry_matter_loss", inplace=True)
        ...
        _df.eval("trips = (2 * feedstock_amount / truck_capacity - 1)", inplace=True)
        ...
        _unp_pm25 = (
            self.fugitive_dust_onroad_constants[
                (self.fugitive_dust_onroad_constants["road_type"] == "unpaved")
                & (self.fugitive_dust_onroad_constants["pollutant"] == "pm2.5")
            ][["constant", "value"]]
            .set_index("constant")
            .to_dict()["value"]
        )
        _unp_pm10 = (
            self.fugitive_dust_onroad_constants[
                (self.fugitive_dust_onroad_constants["road_type"] == "unpaved")
                & (self.fugitive_dust_onroad_constants["pollutant"] == "pm10")
            ][["constant", "value"]]
            .set_index("constant")
            .to_dict()["value"]
        )
        _pav_pm25 = (
            self.fugitive_dust_onroad_constants[
                (self.fugitive_dust_onroad_constants["road_type"] == "paved primary")
                & (self.fugitive_dust_onroad_constants["pollutant"] == "pm2.5")
            ][["constant", "value"]]
            .set_index("constant")
            .to_dict()["value"]
        ...
        # production regions
        _df.eval(
            "unp_pm25 = trips * unp_vmt * @k * (uprsm_pct_silt/12)**@A * (@W/3)**@B",
            global_dict=_unp_pm25,
            inplace=True,
        )
        _df.eval(
            "unp_pm10 = trips * unp_vmt * @k * (uprsm_pct_silt/12)**@A * (@W/3)**@B",
            global_dict=_unp_pm10,
            inplace=True,
        )
        ...
        if self.router is not None:
            ...
            _vmt_by_county_all_routes = _vmt_by_county_all_routes.groupby(
                [
                    "region_transportation",
                    "source_lon",
                    "source_lat",
        ...
        # generated by router
        _df.eval(
            "pav_pm25 = trips * vmt * @k * @s_L**@A * @W**@B", global_dict=_pav_pm25, inplace=True
        )
        _df.eval(
            "pav_pm10 = trips * vmt * @k * @s_L**@A * @W**@B", global_dict=_pav_pm10, inplace=True
        )
        ...
        _fugdust = (
            ...
            .melt(
                ...
                value_vars=["unp_pm25", "unp_pm10", "pav_pm25", "pav_pm10"],
        ...
        _fugdust.loc[
            _fugdust.road_pollutant.isin(["unp_pm25", "unp_pm10"]), "region_transportation"
        ] = ""
        ...
        _fugdust.loc[_fugdust.road_pollutant.isin(["unp_pm25", "unp_pm10"]), "activity"] = (
            "on unpaved road fugitive dust"
        )
        _fugdust.loc[_fugdust.road_pollutant.isin(["pav_pm25", "pav_pm10"]), "activity"] = (
            "on paved road fugitive dust"
        )
