...
class NONROAD(Module):
    def __init__(
        self, config, production, equipment, forestry_feedstock_names, backfill=True, **kvals
    ):
        """
        ...
        self.project_path = os.path.join(
            self.config.get("nonroad_datafiles_path"), self.model_run_title
        )
        ...
        self.irrigation_feedstock_measure_type = self.config.get(
            "irrigation_feedstock_measure_type"
        )
        ...
        self._conn = pymysql.connect(
            host=self.config.get("nonroad_db_host"),
        ...
        self.nonroad_equipment = NONROADEquipment(
            fpath=self.config.get("nonroad_equipment"), backfill=backfill
        )
        ...
        self.region_fips_map = RegionFipsMap(
            fpath=self.config.get("region_fips_map"), backfill=backfill
        )
        ...
        self.production = self.production.merge(
            self.region_fips_map, how="inner", left_on="region_production", right_on="region"
        )
        ...
        _prod_irr_filter = (self.production.feedstock.isin(self.irrigated_feedstock_names)) & (
            self.production.feedstock_measure == self.irrigation_feedstock_measure_type
        )
        ...
        _irrigation_group = self.irrigation.groupby(
            ["feedstock", "state_fips", "equipment_name", "equipment_horsepower", "activity"],
            as_index=False,
        ).sum(min_count=1)[
            [
                "feedstock",
        ...
        # irrigation df
        _prod_irr = self.production[_prod_irr_filter].merge(
            _irrigation_group, on=["feedstock", "state_fips"]
        )

        # calculate the total annual rate for irrigation (hours/acre)
        _prod_irr.eval(
            "total_annual_rate = acreage_fraction * rate * feedstock_amount", inplace=True
        )
        ...
        _prod_irr = _prod_irr.merge(self.state_fips_map, how="inner", on="state_fips")
        ...
        # one with equipment
        self.production = self.production.merge(self.state_fips_map, how="inner", on="state_fips")
        # create filter to select only the feedstock measure used by NONROAD
        _prod_filter = self.production.feedstock_measure == self.feedstock_measure_type
        ...
        _equip_grouped = self.equipment.groupby(
            [
                "feedstock",
                "tillage_type",
                "equipment_group",
        ...
        # average rates
        _max_year = self.equipment.groupby(
            ["feedstock", "tillage_type", "equipment_group"], as_index=False
        ).max()[["feedstock", "tillage_type", "equipment_group", "rotation_year"]]

        # rename rotation year to max rotation year
        _max_year.rename(index=str, columns={"rotation_year": "max_rotation_year"}, inplace=True)

        # rename rate column to total rate
        _equip_grouped.rename(index=str, columns={"rate": "total_rotation_rate"}, inplace=True)
        ...
        _equip_avg = _equip_grouped.merge(
            _max_year, how="left", on=["feedstock", "tillage_type", "equipment_group"]
        )

        # calculate average rate from total rotation rate and max rotation year
        _equip_avg.eval("average_rate = total_rotation_rate / max_rotation_year", inplace=True)

        # merge prod and the equip with average rates on feedstock, tillage
        # type and equipment group
        self.prod_equip_merge = self.production.merge(
            _equip_avg, how="inner", on=["feedstock", "tillage_type", "equipment_group"]
        )

        # calculate total hours for each equipment type - activity type combo
        self.prod_equip_merge.eval(
            "total_annual_rate = feedstock_amount * average_rate", inplace=True
        )
        ...
        self.nr_files = self.prod_equip_merge[
            ["state_abbreviation", "state_fips", "feedstock", "tillage_type", "activity"]
        ].drop_duplicates()
        ...
        if self.encode_names:
            ...
            _feedstocks.sort()
            _feedstock_codes = pd.DataFrame(
                {"feedstock": _feedstocks, "feedstock_code": np.arange(_feedstocks.__len__())}
            )
            ...
            _tillage_types.sort()
            _tillage_type_codes = pd.DataFrame(
                {
                    "tillage_type": _tillage_types,
                    "tillage_type_code": np.arange(_tillage_types.__len__()),
                }
            )
            ...
            _activity_codes = pd.DataFrame(
                {"activity": _activities, "activity_code": np.arange(_activities.__len__())}
            )
            _activity_codes["activity_code"] = "a" + _activity_codes["activity_code"].map(str)

            # encode feedstock, tillage types and activities
            self.nr_files = (
                self.nr_files.merge(_feedstock_codes, how="inner", on="feedstock")
                .merge(_tillage_type_codes, how="inner", on="tillage_type")
                .merge(_activity_codes, how="inner", on="activity")
            )
            ...
            self.nr_files["pop_file_names"] = (
                self.nr_files["state_abbreviation"].map(str)
                + "_"
            ...
            self.nr_files["out_opt_dir_names"] = (
                self.nr_files["feedstock_code"].map(str)
                + "_"
                + self.nr_files["tillage_type_code"].map(str)
                + "_"
        ...
        for i in np.arange(self.nr_files.shape[0]):
            # message files
            _filepath_list = np.append(
                _filepath_list,
                os.path.join(_nr_folders[1], self.nr_files.msg_file_names.iloc[i] + ".msg"),
            )
            # allocate files
            _filepath_list = np.append(
                _filepath_list,
                os.path.join(_nr_folders[2], self.nr_files.alo_file_names.iloc[i] + ".alo"),
            )
            # population files
            _filepath_list = np.append(
                _filepath_list,
                os.path.join(_nr_folders[3], self.nr_files.pop_file_names.iloc[i] + ".pop"),
            )
            # options files
            _filepath_list = np.append(
                _filepath_list,
        ...
        try:
            ...
            raise ValueError(
                "Total file path length for each NONROAD file cannot exceed 60 characters"
            )
        ...
        if self.encode_names:
            LOGGER.debug(
                "saving encoded feedstock, tillage, and activity codes to %s" % self.project_path
            )
            ...
            _feedstock_codes.to_csv(
                os.path.join(self.project_path, "feedstock_codes.csv"), index=False
            )
            _tillage_type_codes.to_csv(
                os.path.join(self.project_path, "tillage_type_codes.csv"), index=False
            )
            _activity_codes.to_csv(
                os.path.join(self.project_path, "activity_codes.csv"), index=False
            )
    ...
    def equipment(self, value):
        ...
        def _validate_hp_ranges():
            _sql = (
                "SELECT CONVERT(SCC, CHAR) AS nonroad_equipment_scc,"
                " MIN(hpMin) AS hp_min,"
                " MAX(hpMax) AS hp_max"
            ...
            _equip = (
                value[["equipment_name", "equipment_horsepower"]]
                .drop_duplicates()
                .merge(
                    self.nonroad_equipment[
                        ["equipment_name", "nonroad_equipment_scc"]
                    ].drop_duplicates(),
                    on="equipment_name",
                )
            ...
            if not _invalid.empty:
                for _row in [list(x) for x in set(tuple(x) for x in _invalid.values.tolist())]:
                    LOGGER.error(
                        "{0} ({2}) horsepower must be between {3} and {4} (currently {1})".format(
                            *_row
                        )
                    )
                return False
...
"""
        ...
        for i in np.arange(self.nr_files.shape[0]):
            _fpath = os.path.join(
                self.project_path, "ALLOCATE", self.nr_files.alo_file_names.iloc[i] + ".alo"
            )
            LOGGER.debug(
                "creating allocation file %s (%s/%s)" % (_fpath, i + 1, self.nr_files.shape[0])
            )
            # initialize file
            with open(_fpath, "w") as _alo_file_path:
                ...
                _prod_filter = (
                    (
                        self.prod_equip_merge.state_abbreviation
                        == self.nr_files.state_abbreviation.iloc[i]
                    )
                    & (self.prod_equip_merge.feedstock == self.nr_files.feedstock.iloc[i])
                    & (self.prod_equip_merge.tillage_type == self.nr_files.tillage_type.iloc[i])
                    & (self.prod_equip_merge.activity == self.nr_files.activity.iloc[i])
                )
                ...
                _indicator_list = (
                    self.prod_equip_merge[_prod_filter][["fips", "feedstock", "feedstock_amount"]]
                    .drop_duplicates()
                    .groupby(["fips", "feedstock"])
                    .sum()
                ...
                for _fips in _indicator_list["fips"].unique():
                    ...
                    if self.forestry_feedstock_names is not None:
                        if self.nr_files.feedstock.iloc[i] in self.forestry_feedstock_names:
                            _ind_code = "LOG"

                            # calculate forestry indicator
                            _ind = (
                                _indicator_list.feedstock_amount[
                                    _indicator_list["fips"] == _fips
                                ].values[0]
                                * 2000.0
                                / 30.0
                            )
    ...
    def create_options_files(self):
        ...
        #  of scenario
        kvals = {
            "episode_year": self.year,
            "model_run_title": self.model_run_title,
            "temp_min": self.temp_min,
            "temp_max": self.temp_max,
            "temp_mean": self.temp_mean,
            "ALLOC_XREF": os.path.join(self.nonroad_path, "data", "allocate", "allocate.xrf"),
            "ACTIVITY": os.path.join(self.nonroad_path, "data", "activity", "activity.dat"),
            "EXH_TECHNOLOGY": os.path.join(self.nonroad_path, "data", "tech", "tech-exh.dat"),
            "EVP_TECHNOLOGY": os.path.join(self.nonroad_path, "data", "tech", "tech-evp.dat"),
            "SEASONALITY": os.path.join(self.nonroad_path, "data", "season", "season.dat"),
            "REGIONS": os.path.join(self.nonroad_path, "data", "season", "season.dat"),
            "EPS2_AMS": "",
            "US_COUNTIES_FIPS": os.path.join(self.nonroad_path, "data", "allocate", "fips.dat"),
            "RETROFIT": "",
            "National_defaults": os.path.join(self.nonroad_path, "data", "growth", "nation.grw"),
            "EMFAC_THC_exhaust": os.path.join(self.nonroad_path, "data", "emsfac", "exhthc.emf"),
            "EMFAC_CO_exhaust": os.path.join(self.nonroad_path, "data", "emsfac", "exhco.emf"),
            "EMFAC_NOX_exhaust": os.path.join(self.nonroad_path, "data", "emsfac", "exhnox.emf"),
            "EMFAC_PM_exhaust": os.path.join(self.nonroad_path, "data", "emsfac", "exhpm.emf"),
            "EMFAC_BSFC": os.path.join(self.nonroad_path, "data", "emsfac", "bsfc.emf"),
            "EMFAC_Crankcase": os.path.join(self.nonroad_path, "data", "emsfac", "crank.emf"),
            "EMFAC_Spillage": os.path.join(self.nonroad_path, "data", "emsfac", "spillage.emf"),
            "EMFAC_Diurnal": os.path.join(self.nonroad_path, "data", "emsfac", "evdiu.emf"),
            "EMFAC_Tank_Perm": os.path.join(self.nonroad_path, "data", "emsfac", "evtank.emf"),
            "EMFAC_Non_RM_Hose_Perm": os.path.join(
                self.nonroad_path, "data", "emsfac", "evhose.emf"
            ),
            "EMFAC_RM_Fill_Neck_Perm": os.path.join(
                self.nonroad_path, "data", "emsfac", "evneck.emf"
            ),
            "EMFAC_RM_Supply_Return": os.path.join(
                self.nonroad_path, "data", "emsfac", "evsupret.emf"
            ),
            "EMFAC_RM_Vent_Perm": os.path.join(self.nonroad_path, "data", "emsfac", "evvent.emf"),
            "EMFAC_Hot_Soaks": os.path.join(self.nonroad_path, "data", "emsfac", "evhotsk.emf"),
            "EMFAC_RuningLoss": os.path.join(self.nonroad_path, "data", "emsfac", "evrunls.emf"),
            "DETERIORATE_THC_exhaust": os.path.join(
                self.nonroad_path, "data", "detfac", "exhthc.det"
            ),
            "DETERIORATE_CO_exhaust": os.path.join(
                self.nonroad_path, "data", "detfac", "exhco.det"
            ),
            "DETERIORATE_NOX_exhaust": os.path.join(
                self.nonroad_path, "data", "detfac", "exhnox.det"
            ),
            "DETERIORATE_PM_exhaust": os.path.join(
                self.nonroad_path, "data", "detfac", "exhpm.det"
            ),
            "DETERIORATE_Diurnal": os.path.join(self.nonroad_path, "data", "detfac", "evdiu.det"),
            "DETERIORATE_Tank_Perm": os.path.join(
                self.nonroad_path, "data", "detfac", "evtank.det"
            ),
            ...
            "DETERIORATE_RM_Vent_Perm": os.path.join(
                self.nonroad_path, "data", "detfac", "evvent.det"
            ),
            "DETERIORATE_Hot_Soaks": os.path.join(
                self.nonroad_path, "data", "detfac", "evhotsk.det"
            ),
            "DETERIORATE_RuningLoss": os.path.join(
                self.nonroad_path, "data", "detfac", "evrunls.det"
            ),
            "EXHAUST_BMY_OUT": "",
            "EVAP_BMY_OUT": "",
            "SI_report_file_CSV": os.path.join(self.project_path, "MESSAGES", "NRPOLLUT.csv"),
            "DAILY_TEMPS_RVP": "",
        }
...
"""
        ...
        for i in np.arange(self.nr_files.shape[0]):
            kvals_fips = {
                "state_fips": "{fips:0<5}".format(fips=self.nr_files.state_fips.iloc[i]),
                "MESSAGE": os.path.join(
                    self.project_path, "MESSAGES", self.nr_files.msg_file_names.iloc[i] + ".msg"
                ),
                ...
                "Population_File": os.path.join(
                    self.project_path, "POP", self.nr_files.pop_file_names.iloc[i] + ".pop"
                ),
                "Harvested_acres": os.path.join(
                    self.project_path, "ALLOCATE", self.nr_files.alo_file_names.iloc[i] + ".alo"
                ),
            }
            ...
            with open(f, "w") as _opt_file:
                _opt_file.writelines(_options_file_template.format(**kvals, **kvals_fips))
    ...
    def _write_population_file_line(self, df, _pop_file):
        ...
        _string_to_write = (
            "{fips:0>5} {sub_reg:>5} {year:>4} {scc_code:>10}"
    ...
    def create_population_files(self):
        ...
        _nrsourceusetype.rename(
            index=str, columns={"medianLifeFullLoad": "equipment_lifetime"}, inplace=True
        )

        # filter down the nrsourceusetype table based on the list of
        # equipment in user-provided nonroad_equipment
        _scc_filter = _nrsourceusetype.SCC.isin(self.nonroad_equipment.nonroad_equipment_scc)
        ...
        _nr_pop_info = (
            self.nonroad_equipment.dropna()
            .merge(_nrsourceusetype, how="inner", left_on="nonroad_equipment_scc", right_on="SCC")
            .merge(_nrhprangebin, how="inner", on="NRHPRangeBinID")
        )

        _nr_equip_filter = self.prod_equip_merge[
            ["equipment_name", "equipment_horsepower"]
        ].drop_duplicates()

        _nr_pop_info_filtered = _nr_pop_info.merge(
            _nr_equip_filter, how="inner", on="equipment_name"
        )
        ...
        _hp_filter = (_nr_pop_info_filtered.equipment_horsepower >= _nr_pop_info_filtered.hpMin) & (
            _nr_pop_info_filtered.equipment_horsepower <= _nr_pop_info_filtered.hpMax
        )
        ...
        _nr_pop_equip_merge = self.prod_equip_merge.merge(
            _nr_pop_info_filtered, how="left", on=["equipment_name", "equipment_horsepower"]
        )
        ...
        # defaults)
        _nr_pop_equip_merge.eval(
            "equipment_population = total_annual_rate / hoursUsedPerYear", inplace=True
        )
        ...
        _nr_pop = _nr_pop_equip_merge.groupby(
            [
                "state_abbreviation",
                "feedstock",
                "tillage_type",
...
"""
        ...
        for i in np.arange(self.nr_files.shape[0]):
            ...
            _pop_path = os.path.join(_pop_dir, self.nr_files.pop_file_names.iloc[i] + ".pop")

            LOGGER.debug(
                "creating %s population file (%s/%s)" % (_pop_path, i + 1, self.nr_files.shape[0])
            )
            # create filter to pull out only the lines in _nr_pop that are
            # relevant to this population file
            _nr_pop_filter = (
                (_nr_pop.state_abbreviation == self.nr_files.state_abbreviation.iloc[i])
                & (_nr_pop.feedstock == self.nr_files.feedstock.iloc[i])
                & (_nr_pop.tillage_type == self.nr_files.tillage_type.iloc[i])
            ...
            with open(_pop_path, "w") as _pop_file:
                ...
                _nr_pop_sub.apply(func=self._write_population_file_line, axis=1, args=(_pop_file,))
    ...
    def create_batch_files(self):
        ...
        kvals["nonroad_exe_path"] = os.path.join(self.nonroad_path, self.nonroad_exe)
        ...
        for idx, i in enumerate(_files):
            ...
            with open(_batch_filepath, "w") as _batch_file:
                ...
                for j in np.arange(nr_files_sub.shape[0]):
                    ...
                    # to this batch file
                    kvals["opt_filepath"] = os.path.join(
                        _batch_path,
        ...
        self.master_batch_filepath = os.path.join(_batch_path, self.model_run_title + ".bat")
        ...
        with open(self.master_batch_filepath, "w") as _master_batch_file:
            ...
            for _file in os.listdir(_batch_path):
                ...
                if _file.endswith(".bat") & ~_file.startswith(self.model_run_title):
    ...
    def postprocess(self):
        ...
        for i in np.arange(self.nr_files.shape[0]):
            ...
            if os.path.isfile(_nr_out_file):
                ...
                #  names
                _to_append = pd.read_csv(
                    _nr_out_file,
                    sep=",",
                    header=9,
                    usecols=[0, 5, 6, 7, 8, 9, 10, 19],
                    names=["fips", "thc", "co", "nox", "co2", "so2", "pm", "fuel"],
                    dtype={
                        "fips": np.str,
                        "thc": np.float,
                        "co": np.float,
        ...
        _nr_out = _nr_out.groupby(
            ["feedstock", "fips", "tillage_type", "activity"], as_index=False
        ).sum()

        # use the nonroad fips-region map to get back to region_production
        _nr_out = _nr_out.merge(self.region_fips_map, how="inner", on="fips")

        # rename region column from region-fips map to region_production
        _nr_out.rename(index=str, columns={"region": "region_production"}, inplace=True)

        # melt the nonroad output to put pollutant names in one column and
        # pollutant amounts in a second column
        _nr_out_melted = _nr_out.melt(
            id_vars=["feedstock", "region_production", "tillage_type", "activity"],
            value_vars=["co", "nox", "co2", "so2", "pm10", "pm25", "voc", "nh3"],
            var_name="pollutant",
            value_name="pollutant_amount",
        )

        # nonroad emissions are calculated in short tons; convert to pounds
        _nr_out_melted["pollutant_amount"] = (
            _nr_out_melted["pollutant_amount"] * self.conversion_factors["ton"]["pound"]
        )
