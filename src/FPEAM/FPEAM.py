...
class FPEAM(object):
    ...
    MODULES = {
        "emissionfactors": EngineModules.EmissionFactors,
    ...
    def __init__(self, run_config):
        ...
        for _module in _modules:
            try:
                ...
                _module_error = True
                LOGGER.error(
                    "invalid module name: %s; must be one of: %s"
                    % (_module, ", ".join(list(self.MODULES.keys())))
                )
            else:
        ...
        self.equipment = (
            Data.Equipment(
                fpath=self.config.get("equipment"), backfill=self.config.as_bool("backfill")
            )
            .reset_index()
        ...
        self.production = (
            Data.Production(
                fpath=self.config.get("production"), backfill=self.config.as_bool("backfill")
            )
            .reset_index()
            .rename({"index": "row_id"}, axis=1)
        )
        self.feedstock_loss_factors = (
            Data.FeedstockLossFactors(
        ...
        self.truck_capacity = (
            Data.TruckCapacity(
                fpath=self.config.get("truck_capacity"), backfill=self.config.as_bool("backfill")
            )
            .reset_index()
            .rename({"index": "row_id"}, axis=1)
        )
        ...
        if self.config.as_bool("use_router_engine") and (
            "MOVES" in self._modules.keys() or "fugitivedust" in self._modules.keys()
        ):
            _transportation_graph = Data.TransportationGraph(
                fpath=self.config.get("transportation_graph"),
                backfill=self.config.as_bool("backfill"),
            )
            _node_locations = Data.TransportationNodeLocations(
                fpath=self.config.get("node_locations"), backfill=self.config.as_bool("backfill")
            )
            ...
            if not _transportation_graph.empty and not _node_locations.empty:
                ...
                self.router = Router(
                    edges=_transportation_graph, node_map=_node_locations, memory=self.memory
                )
        else:
        ...
        for _module in _modules:
            _config = (
                run_config.get(_module.lower(), None)
                or load_configs(
                    str(
                        _pkg_files("FPEAM").joinpath("%s/%s.ini" % (CONFIG_FOLDER, _module.lower()))
                    )
                )[_module.lower()]
            )
            _config["scenario_name"] = _config.get("scenario_name", "").strip() or self.config.get(
                "scenario_name"
            )
            ...
            try:
                ...
                if _module not in FPEAM.MODULES.keys():
                    LOGGER.warning(
                        "invalid module name: {}. Must be one of: {}".format(
                            _module, FPEAM.MODULES.keys()
                        )
                    )
                else:
                    raise
    ...
    def collect(self, modules=None):
        ...
        _loss_factors = (
            _loss_factors[["feedstock", "dry_matter_remaining"]]
        ...
        _loss_factors_farmgate = (
            _loss_factors_farmgate[["feedstock", "dry_matter_remaining"]]
            .groupby(["feedstock"], as_index=False)
            .prod(numeric_only=True)[["feedstock", "dry_matter_remaining"]]
        )
        ...
        _prod_row_filter = _prod["feedstock_measure"].isin(["harvested", "production"]).values

        # delete the columns from prod that don't need to be merged into
        # results
        del _prod["region_destination"], _prod["equipment_group"], _prod["row_id"]

        # rename the feedstock production units columns to avoid confusion
        # with the pollutant units columns in the results
        _prod = _prod.rename(
            columns={
                "unit_numerator": "feedstock_unit_numerator",
        ...
        _prod_losses_farmgate = _prod_losses_farmgate.merge(_loss_factors_farmgate, on="feedstock")

        # recalculate the feedstock amounts to account for losses
        _prod_losses["feedstock_amount"] = (
            _prod_losses.feedstock_amount * _prod_losses.dry_matter_remaining
        )
        _prod_losses_farmgate["feedstock_amount"] = (
            _prod_losses_farmgate.feedstock_amount * _prod_losses_farmgate.dry_matter_remaining
        )
        ...
        _df = _df_modules.merge(
            _prod_filtered, on=("feedstock", "tillage_type", "region_production")
        )
    ...
    def summarize(self):
        ...
        _summarize_by_region_production = self.results.groupby(
            ["feedstock", "tillage_type", "region_production", "pollutant"], as_index=False
        ).sum()

        _summarize_by_region_production["unit_numerator"] = (
            "lb pollutant"  # @TODO: should we be reading this in from the data?
        )
        _summarize_by_region_production["unit_denominator"] = "county-year"

        _summarize_by_region_production[
            [
                "feedstock",
                "tillage_type",
                "region_production",
        ...
        self.summaries["by_region_production"] = _summarize_by_region_production[
            [
                "feedstock",
                "tillage_type",
                "region_production",
        ...
        if self.config["fpeam"].get("inmap_county_export", False) is True:
            ...
            _shp_fpath_in = str(
                _pkg_files("FPEAM").joinpath("data/inputs/tl_2019_us_county/tl_2019_us_county.shp")
            )
            _df = geopandas.read_file(
                _shp_fpath_in, dtype={"STATEFP": str, "COUNTYFP": str, "geometry": MultiPolygon}
            )[["STATEFP", "COUNTYFP", "NAME", "geometry"]]
            _df["region_production"] = _df.STATEFP + _df.COUNTYFP

            _grp_df = (
                _summarize_by_region_production[
                    ["region_production", "pollutant", "pollutant_amount"]
                ]
                .groupby(["region_production", "pollutant"], as_index=False)
                .sum()
            )
            _grp_df_pivot = _grp_df.pivot(
                index="region_production", columns="pollutant", values="pollutant_amount"
            )

            _gdf_join = _df[["region_production", "NAME", "geometry"]].join(
                other=_grp_df_pivot, on="region_production", how="right"
            )

            _gdf_join.rename(
                columns={
                    "region_production": "cnty_fips",
                    "NAME": "cnty_name",
                    "co": "CO",
                    "nh3": "NH3",
                    "nox": "NOx",
        ...
        _results_to_normalize.eval(
            "normalized_pollutant_amount = pollutant_amount / feedstock_amount", inplace=True
        )

        # add unit columns for normalized pollutant amounts
        _results_to_normalize["normalized_pollutant_unit_numerator"] = _results_to_normalize[
            "unit_numerator"
        ]
        _results_to_normalize["normalized_pollutant_unit_denominator"] = _results_to_normalize[
            "feedstock_unit_numerator"
        ]

        # sum normalized pollutant amounts over modules and activities
        _results_normalized = _results_to_normalize.groupby(
            [
                "feedstock",
                "feedstock_measure",
                "tillage_type",
        ...
        if "region_transportation" in self.results.columns:
            ...
            _summarize_by_region_transportation = self.results.groupby(
                ["feedstock", "tillage_type", "region_transportation", "pollutant"], as_index=False
            ).sum()
            ...
            _summarize_by_region_transportation[
                [
                    "feedstock",
                    "tillage_type",
        ...
        _summarize_by_module = self.results.groupby(
            ["feedstock", "tillage_type", "module", "pollutant"], as_index=False
        ).sum()
        ...
        _fpath_summary = os.path.join(
            self.config.get("project_path"),
            "%s_total_emissions_by_module.csv" % self.config.get("scenario_name"),
        )

        _summarize_by_module[
            ["feedstock", "tillage_type", "module", "pollutant", "pollutant_amount"]
        ].to_csv(_fpath_summary, index=False)
