...
def _parse_bytes(xml_bytes):
    ...
    return etree.fromstring(
        xml_bytes.split(b"?>", 1)[-1].strip() if xml_bytes.startswith(b"<?") else xml_bytes
    )
...
def _build_import_xml(moves_version):
    ...
    for rid, rname in [
        ...
        etree.SubElement(
            roadtypes, "roadtype", roadtypeid=rid, roadtypename=rname, modelCombination="M1"
        )
    ...
    databasesel = etree.Element(
        "databaseselection",
        servername="localhost",
        databasename="fips_26161_2017_MOVES5_in",
        description="",
    )
    ...
    if moves_version.startswith("MOVES3"):
        importfilestring = E.moves(
            E.importer(
                ...
                E.fuel(
                    _cdata(),
                    E.parts(
                        E.FuelSupply(fuelsupfile),
                        E.FuelFormulation(fuelformfile),
                        E.FuelUsageFraction(fuelusagefile),
                        E.AVFT(avftfile),
                    ),
                ),
                ...
                E.vehicletypevmt(
                    _cdata(),
                    E.parts(
                        E.HPMSVtypeYear(HPMSyearfile),
                        E.monthVMTFraction(monthVMTfile),
                        E.dayVMTFraction(dayVMTfile),
                        E.hourVMTFraction(hourVMTfile),
                    ),
                ),
                E.hotelling(
                    _cdata(),
                ...
                E.generic(
                    _cdata(), E.parts(E.anytable(E.tablename("agecategory"), E.filename("")))
                ),
                mode="county",
            )
        )
        ...
        importfilestring = E.moves(
            E.importer(
                ...
                E.onroadretrofit(_cdata(), E.parts(E.onRoadRetrofit(E.filename("")))),
                E.generic(
                    _cdata(), E.parts(E.anytable(E.tablename("activitytype"), E.filename("")))
                ),
                mode="county",
            )
        )
...
class TestMOVES5ImportXML:
    ...
    def test_no_rampfraction(self):
        ...
        root = _parse_bytes(xml_bytes)
        importer = root.find("importer")
        assert importer.find("rampfraction") is None, "MOVES5 should not have rampfraction section"
...
def _make_moves_stub(source_type_id=61, vmt_fraction=None):
    ...
    _vf = {
        2: vmt_fraction["rural_restricted"],
...
def _apply_roadtype_logic(stub, zrt_df, fips="19109"):
    ...
    if not zrt_df.empty and _total_sho > 0:
        ...
        result = _zrt[["sourceTypeID", "roadTypeID", "roadTypeVMTFraction"]].reset_index(drop=True)
    else:
        result = stub.roadtypevmt[["sourceTypeID", "roadTypeID", "roadTypeVMTFraction"]].copy()
    return result
...
class TestCountyRoadTypeFractions:
    ...
    def test_county_fractions_reflect_rural_dominated_county(self):
        ...
        assert r3_frac > 0.85, (
            f"Rural unrestricted fraction should be >85% for rural Iowa, got {r3_frac:.4f}"
        )
    ...
    def test_county_fractions_reflect_urban_dominated_county(self):
        ...
        stub = _make_moves_stub()
        result = _apply_roadtype_logic(stub, zrt)
        urban_frac = result.loc[result["roadTypeID"].isin([4, 5]), "roadTypeVMTFraction"].sum()
    ...
    def test_fallback_to_national_default_when_empty(self):
        ...
        stub = _make_moves_stub()
        result = _apply_roadtype_logic(stub, pd.DataFrame(columns=["roadTypeID", "SHOAllocFactor"]))
